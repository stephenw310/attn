...
- The test-only `attn:test:failNextAction` and `attn:test:failNextActionAuth` events install a seeded provider that rejects only the named thread once, serves authoritative snapshots for every fixture thread while installed, and clears after the target recovery read or successful auth retry. T18 e2e covers permanent recovery, auth-pause UI, SQLite convergence, acknowledged IPC toast delivery, and undo invalidation without contacting Gmail.
