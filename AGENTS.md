...
- Composer/outbox specs share the `ComposerPage` driver in `e2e/composer.ts`: open with `openNew()`/`openReply()`, inspect normalized recipient chips through their `data-email`, type through the editor, send with the platform modifier, and read the footer pending state without coupling tests to composer markup.
- Time-driven main-process schedulers take the injectable `SchedulerTime` contract from `src/main/time.ts` (`now()` plus a timer factory). Unit tests supply that dependency or Vitest fake timers; never make undo-send or outbox tests wait on wall-clock time.
