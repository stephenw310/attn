...
- The test-only `attn:test:failNextAction` event installs a seeded one-action provider that rejects the named thread permanently, then returns that thread's original fixture snapshot to the production failed-action recovery path. It is cleared after the batched revert notice, so T18 e2e covers the real executor, SQLite convergence, IPC toast, and undo invalidation without contacting Gmail.
