...
**Visual self-check:** the e2e suite rewrites `e2e/.artifacts/login.png`, `inbox.png`, `inbox-light.png`, `reading.png`, `reading-light.png`, `simple-mail.png`, `mail-layout.png`, `mail-layout-light.png`, `neutral-backgrounds-light.png`, `colored-reply.png`, `label-picker.png`, `auth-paused.png`, `composer.png`, `inline-reply.png`, `draft-chip.png`, `attachments.png`, `newsletter-quote.png`, and `gmail-draft.png` (grep `e2e/*.spec.ts` for `.artifacts` when adding one, and list it here). After UI changes, inspect every affected artifact and confirm the rendering matches intent; test setup must not leave text-selection highlights in screenshots. Failure debugging: traces land in `e2e/.results/` (`npx playwright show-trace …`), and the main-process log is attached to failed tests.
