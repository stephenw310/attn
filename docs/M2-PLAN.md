...
2. **Auth failures never revert.** A 401/revoked token doesn't mean "Gmail rejected this", it means "we couldn't ask". The intent is still valid, so those rows pause behind a typed stored auth marker and re-pend after the user chooses the visible **Reconnect Google** action and completes successful sign-in for the same account. Pre-T18 message-formatted 401 rows are recognized once at executor construction and normalized to the typed marker.
