...
When one thread from a bulk action is rejected, only that thread's queued-row reference and inverse are removed from the undo entry; the unaffected threads remain undoable.
