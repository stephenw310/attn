...
- Convert pasted headings to sized, bold paragraphs, code to monospace text, and checklists to checked/unchecked symbols. Expand toggles into readable content. Convert supported embedded media URLs to links. Unsupported layouts keep the existing sanitized preservation path. Clipboard conversion does not rewrite stored drafts or Gmail imports.
- `Mod+Shift+V` and the Paste without formatting palette command insert clipboard text using the current text style. Keep line breaks and insert markup characters literally. Clear formatting in the formatting menu and palette removes selected text styling and links while preserving text and block structure. A collapsed selection clears the style for subsequent typing. Both operations support undo.
