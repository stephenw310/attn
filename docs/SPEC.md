...
command is registered, and group equivalent keys under one label. Pressing a chord prefix such as `G`
temporarily replaces that line with the valid next keys derived from the same command registry. The guide
