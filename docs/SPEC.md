...
- **Reader layout:** conversation messages use flat rows with separators. Expanded headers have no filled banner. The active message has a short accent line and an outlined avatar in both expanded and collapsed states. `N` and `P` move that marker; `O` expands or collapses the active message. Expand all messages and Collapse all messages affect readable messages without revealing hidden Trash copies. These controls are also available through the command palette. Expanded messages expose Reply, Reply all, and Forward actions when no draft is open. The reader summary shows message count, participants, and current user-label chips. Back to the originating list shows an Escape keycap only when no inline draft is open. The reader hint bar shows N/P, O, and R without duplicating Escape.
