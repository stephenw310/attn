...
calls Gmail `users.threads.modify` under the existing `gmail.modify` grant.
...
change, so its due time cannot add `INBOX` again. Undo restores both the reminder and the exact prior labels.
