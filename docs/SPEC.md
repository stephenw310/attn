...
subsets. Still M3: the utility-process move, the existence-sweep tombstone pass for label-less orphans, and
the mailbox/search surfaces (§9 #10, #17).
...
- The full-width list groups conversations under relative date headings (Today, Yesterday, Last 7 days, then older periods) and shows sender(s), subject, a 1–2 line snippet, timestamp, and chips (attachment, starred, snoozed-return, follow-up). Unread rows are visually distinct. Lists at 500 rows or more use fixed-height windowing with overscan; Inbox and Snoozed expose a measured 10,000-row maximum.
...
- **M3 — Find & focus.** Opens with the sync restructure the utility-process move already implies (§6, §9 #17), planned as S1–S4 in docs/M3-PLAN.md. The all-mail and spam-trash backfill stages and per-label membership reconciliation shipped early in M2's #51. Per-message label storage followed in S2. The remaining sync work is the utility-process move (S1) and the existence-sweep tombstone pass for expiry recovery (S4). Then: FTS5 instant search + operators, system mailbox navigation (Inbox/All Mail/Sent/Drafts/Starred/Snoozed/Spam/Trash), split inbox + rules, inbox-zero states, themes, command palette hardened (every command registered), and the contextual chord guide (§9 #14).
...
19. **The utility process owns SQLite (2026-08-22):** §6 commits M3 to moving Gmail fetch, backfill, derived-data rebuilds, and FTS indexing into a utility process, and left open which side keeps the database. The utility process holds the only connection and is the only writer; the main process asks it for reads. Two writers is a corruption bug, so there is no path where main writes directly. Accepted costs, recorded rather than discovered: every renderer read becomes a two-hop round trip, so §7's interaction budgets are re-proved under the new boundary rather than assumed, with cached conversation open at 50 ms the tightest; and the outbox and action executors, which carry the exactly-once invariant, need an explicit home decided before any code moves, because an IPC hop in front of a write introduces a failure mode M2 does not have. The main process runs raw SQL at 81 sites across 20 files today, which is the size of the change. Planned as M3's S1 in docs/M3-PLAN.md.
