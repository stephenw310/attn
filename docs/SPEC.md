...
same Spam and Trash deltas and the same Gmail operation as those Move destinations. This keeps each change
atomic. Those shortcuts also cancel pending snooze reminders. Undo reverses the applied thread-label delta
and restores the prior reminder exactly; permanent-failure recovery restores the same local snapshot.
