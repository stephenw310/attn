...
*Shipped staging:* M2 currently runs inbox → bodies → sent → reconcile; the drafts stage lands with T14D and
the lifetime sweep with T13A (both M2). The all-mail and spam-trash stages, per-message label storage, and
the generalized reconcile open M3 (§9 #10, #17).
...
- **Drafts:** autosaved to the local store one second after typing stops, and at least every five seconds while typing continues, so a force-quit loses at most five seconds of work. The **Gmail Drafts mirror is a separate, slower schedule** — debounced three seconds after typing stops, skipped entirely when nothing changed since the last push — so a composing session produces a handful of `drafts.update` calls rather than one per second. Drafts are listed in the Drafts view (`G` `D`) and reopenable, thread-bound drafts are marked on their conversation row, several may exist at once, and at most one reply draft and one forward draft exist per conversation.
...
- **M3 — Find & focus.** Opens with the sync restructure the utility-process move already implies (§6, §9 #17): the all-mail and spam-trash backfill stages, per-message label storage, and generalized per-label reconcile/expiry recovery. Then: FTS5 instant search + operators, system mailbox navigation (Inbox/All Mail/Sent/Drafts/Starred/Snoozed/Spam/Trash), split inbox + rules, inbox-zero states, themes, command palette hardened (every command registered).
