...
**Packaging:** `electron-builder`; auto-update via GitHub Releases. macOS notarization + Windows code signing required for public distribution (skippable for personal builds). *Status:* personal-build packaging shipped early, at M1 exit — a manually dispatched GitHub Actions workflow produces macOS DMG/ZIP for both architectures (ad-hoc signed) and a Windows NSIS installer (unsigned), each verified by `npm run package:verify`. M4's T39 then shipped the credential-independent half of auto-update: the schema-gated update state machine, the personal/release distribution metadata, the construction gate that keeps personal, dev, and seeded builds off any feed, and `package:verify --release`. The publishing workflow followed on 2026-09-04 (`.github/workflows/release.yml`, [docs/RELEASE.md](RELEASE.md)): a `v*` tag builds, signs, notarizes, and verifies on both OS runners, stamps `requiredSchemaVersion` into each feed file, and publishes to the feed named by the `ATTN_RELEASE_FEED` repository variable; Settings → About shows the version, build kind, feed, and last check with `Check for updates` and `Restart to update`. Real signing and notarization remain open operator work — they need Apple Developer ID and notarytool credentials, a Windows signing certificate, and the release-feed repository decision (a public repository, since the updater reads the feed anonymously and this repository is private). T40 tracks that evidence.
