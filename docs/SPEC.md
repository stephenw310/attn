# Attn — Product & Technical Spec (v0.18)
...
utility process. The mailbox surfaces shipped with T22, and search followed in T23–T25 (§9 #17), so every
planned M3 task is complete.
...
- **M1 staging:** before splits exist, notifications and the badge cover every unread Inbox conversation.
  Per-split filtering arrives with F11 in M3.
...
**Packaging:** `electron-builder`; auto-update via GitHub Releases. macOS notarization + Windows code signing required for public distribution (skippable for personal builds). *Status:* personal-build packaging shipped early, at M1 exit — a manually dispatched GitHub Actions workflow produces macOS DMG/ZIP for both architectures (ad-hoc signed) and a Windows NSIS installer (unsigned), each verified by `npm run package:verify`. M4's T39 then shipped the credential-independent half of auto-update: the update state machine, the personal/release distribution metadata, the construction gate that keeps personal, dev, and seeded builds off any feed, and `package:verify --release`. The publishing workflow followed on 2026-09-04 (`.github/workflows/release.yml`, [docs/RELEASE.md](RELEASE.md)): a `v*` tag builds, signs, notarizes, and verifies on both OS runners, stamps the target and minimum migratable schemas into each feed file, and publishes to the feed named by the `ATTN_RELEASE_FEED` repository variable; Settings → About shows the version, build kind, feed, and last check with `Check for updates` and `Restart to update`. Runtime migration support followed in PR #114: a release can update a profile from any schema retained by its ordered migration registry. Real signing and notarization remain open operator work — they need Apple Developer ID and notarytool credentials, a Windows signing certificate, and the release-feed repository decision (a public repository, since the updater reads the feed anonymously and this repository is private). T40 tracks that evidence.

M4 keeps personal packaging available without signing credentials and disables its updater, even when
packaged. Public-release builds require explicit release metadata and signature verification. The rolling
feed declares both the target schema and the oldest schema the release can migrate. The updater checks
that range before download and installation. At startup, `openDatabase()` runs every skipped migration in
one transaction and checks integrity before commit. Missing or incompatible metadata rejects the update
without replacing the app or changing local data. Every schema change adds one immutable migration; an
upgrade never deletes a profile or asks the user to edit `user_version` manually.
...
UI polish (2026-09-05): the Done toast says `Marked done`, the split manager uses a columns icon beside
the tabs, and the title bar omits the unread-progress meter. Split labels and count slots keep stable
geometry across selection and count changes. Dark/light Electron coverage and screenshots are in
`e2e/splits.spec.ts`; pending-action and Outbox controls remain available.

Usage fixes (2026-09-05): notification clicks locate archived and moved conversations, notification
banners omit message bodies, account totals no longer wait for list responses, and Spam/Trash list
queries exclude message bodies from intermediate rows and seek messages from the sparse label set.
Unloaded mailboxes show loading rather than an empty state. Regression coverage is recorded in
[M5 A3 and A4](M5-PLAN.md).
...
**Status (2026-09-03, M4):** M4's feature work is merged on `main` (#101, with the Windows badge revision in #104): the settings surface with the keyboard cheat sheet and the notification-focus fix (T32), the per-account historical sync limit (T32A), the optional "Sent with Attn" footer (T32B), remote-image control with the CSP-layered request filter (T33), snippets (T34), follow-up reminders with durable send ordering on schema v25 (T35), the AI writing foundation with safeStorage key custody and the fake-provider seam (T36), Mod+J reply drafting with streaming, one-step undo, and inline refine (T37), separately consented inline autocomplete with its bounded authored-body excerpt and main-process rate limits (T37A), the large numeric Windows unread overlay plus cross-platform badge setting (T38), and T39's credential-independent auto-update half: the schema-gated update state machine, the personal/release distribution metadata, the construction gate that keeps personal, dev, and seeded builds from ever touching a feed, and `package:verify --release`. docs/M4-PLAN.md carries each task's as-shipped status and [T40-EVIDENCE.md](T40-EVIDENCE.md) the exit checklist: still open for v1 sign-off are the operator-credentialed signing/notarization and release-feed decision, the real-OS and real-Gmail manual runs (M1/M2's inherited items included), real-provider AI observations, and M5's A7 dogfood observation. On-hardware perf paint metrics were recorded on 2026-09-03 and passed.

**v1 state (2026-09-02):** every planned engineering task from M0 through M5 is merged on `main`. No
milestone has been formally signed off and no v1 tag exists, because each exit checklist ends in items the
harness cannot run, and those have compounded forward into [T40-EVIDENCE.md](T40-EVIDENCE.md): M1's
real-OS notification click-through, M2's E1–E7, M4's nine open feature boxes, and M5's A7. They fall into three
...
The tightest of these is M2's E1, the one-week sole-client dogfood run, open since M2 and not yet
...
- **M4 — Power finish.** Snippets, follow-up reminders, the optional "Sent with Attn" signature footer (F6), AI reply drafting and inline autocomplete (F17), settings surface, badge polish, auto-update + signing/notarization (personal-build packaging shipped early, at M1 exit — §6 Packaging). Task plan: docs/M4-PLAN.md. *Feature work merged to `main` 2026-09-01 (#101, #104); the operator-credentialed signing/release half of T39 and T40's manual sign-off evidence remain open (see the M4 status above).*
