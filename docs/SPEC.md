...
mailboxes use `I/A/T/D/S/H/P/R/O`, while `1`–`9` maps to Inbox splits in configured order. The guide remains
visible until completion, `Esc`, a view change, or a 2-second timeout. The command palette (`Mod+K`) and
cheat sheet (`Mod+/`) remain the exhaustive discovery surfaces.
