...
Google Contacts import, send-as alias administration, Gmail-native snooze, and Gmail filter administration are not supported. Contact autocomplete derives from mail headers. Gmail confidential-mode messages can contain only API placeholders.
...
- **From identity:** every draft belongs to exactly one account. The From selector lists that account's
  primary address and verified Gmail send-as identities. Configure identities in Gmail settings.
  New messages use the cached Gmail default identity, with the primary address as the offline fallback.
  Replies, reply-all, and forwards match the source message against the account's verified identities.
  An owned From address takes precedence, followed by the first matching To, Cc, or Bcc address.
  ...
  Sends use the chosen identity's display name and Reply-To. Attn never substitutes another sender.
  Replies exclude all verified identities from self-recipients. Draft ownership remains unchanged.
  Identity lists refresh with account sync and remain available offline. SMTP configuration stays in Gmail.
- **Gmail signature:** Attn caches send-as signatures under the existing `gmail.modify` grant and
  ...
  through this resource. Attn uses the initially selected identity's signature for each local composer and does not invent a
...
- Disabling autocomplete cancels its timers and requests and clears previews without disabling explicit
  reply drafting. The master AI switch stops both features; removing the key also disables both. Neither
  switch reaches smart splits, which hold their own consent and their own key. Cancel
...
- **Smart splits keep a separate consent (F11).** They stay off until the user accepts their own disclosure
  and saves a TypeSafe key, which lives in its own `safeStorage` file. Enabling AI writing or autocomplete
  never enables them, and their consent enables no writing feature. The enable screen names what leaves the
  machine: for each Inbox conversation, the subject, the sender's name and address, the recipient count,
...
- Smart splits disabled, or their key absent → zero requests to TypeSafe, including after relaunch.
  Enabling AI writing or autocomplete sends no TypeSafe request. Accepting the smart-splits disclosure
  sends no writing-provider request.
- Settings and a palette command expose the smart-splits consent, its key, and its model. Removing that key
  turns the consent off and leaves AI writing enabled.
