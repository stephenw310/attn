...
- **M3 — Find & focus.** Opens with the sync restructure planned as S1–S4 in docs/M3-PLAN.md. The all-mail and spam-trash backfill stages and per-label membership reconciliation (S3 and half of S4) shipped early, in M2's #51; the utility-process move (S1) is complete. What remains is per-message label storage (S2) and the existence-sweep tombstone pass for expiry recovery (S4). Then: FTS5 instant search + operators, system mailbox navigation (Inbox/All Mail/Sent/Drafts/Starred/Snoozed/Spam/Trash), split inbox + rules, inbox-zero states, themes, command palette hardened (every command registered), and the contextual chord guide (§9 #14).
