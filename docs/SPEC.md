...
- Navigate: `Tab` moves to the next split, and `Shift+Tab` moves to the previous split. Navigation wraps at
  both ends. Outside Inbox, `Tab` returns to Inbox. `←` and `→` remain split-navigation aliases. `G` then
  `1`–`9` jumps by configured split order. Each split
