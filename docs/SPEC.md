...
- Resolve complex clipboard styles with Chromium in an isolated, scriptless frame that blocks resource loads. Snapshot the resulting styles and generated text before removing stylesheets. Preserve unsupported block presentation through the existing read-only content path.
