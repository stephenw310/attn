...
- Convert pasted headings to sized, bold paragraphs, code to monospace text, and checklists to checked/unchecked symbols. Expand toggles into readable content. Convert supported embedded media URLs to links. Unrecognized stylesheets paste as editable plain text, preferably from the clipboard text representation. Do not emulate generated content, responsive CSS, or pseudo-elements. Unsupported inline layouts keep the existing sanitized preservation path; visual fidelity is not guaranteed. Clipboard conversion does not rewrite stored drafts or Gmail imports.
