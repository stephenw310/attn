...
| **Move** (`V`) | Opens a one-shot destination picker for Done or an existing user label. Move removes `INBOX`, adds the chosen label, and closes the picker. From a user-label view, it also removes that view's label. If the thread has a pending snooze reminder, Move cancels it. |
...
chooses one destination, applies it once, and closes the picker. Move preserves unrelated user labels plus
`STARRED`, `UNREAD`, and `SENT`. Choosing Done adds no label. It removes `INBOX` and, from a user-label
view, removes that view's label. The active user label is not offered as a destination because Done is the
explicit removal path.

Move works in Inbox, Inbox splits, All Mail, Sent, Starred, user-label views, and ordinary search results,
including a reader opened from those views. From Inbox or a user-label view, the moved row leaves the view
and the selection or reader advances. In All Mail, Sent, Starred, or a search whose query still matches, the
row remains selected. Move is unavailable in Drafts, Snoozed, Spam, Trash, Outbox, and searches scoped to
those views. A pending-snooze thread can still appear in an allowed view. Moving that thread cancels its
reminder in the same local transaction as the label change, so its due time cannot add `INBOX` again. Undo
restores both the reminder and the exact prior labels. The Snoozed view keeps its dedicated reschedule and
unsnooze actions; Spam and Trash keep their restore actions.
...
- `V` moves a bulk selection with one destination choice, and one `Z` restores each thread's exact prior
  `INBOX` and user-label membership. If a target had a pending snooze reminder, Move cancels it and the same
  `Z` restores its prior state and due time.
...
| `!` | Spam |
| `L` | Label picker |
| `V` | Move to Done or a user label |
...
20. **Move is separate from Label (2026-08-27):** `V` chooses one existing user label or Done, removes `INBOX`, and removes the active user label when invoked from a user-label view. `L` remains the multi-label membership editor. Move preserves unrelated labels and status flags. A pending-snooze thread reached through an allowed view is movable; Move cancels its reminder, and undo restores the reminder with its prior due time. Drafts, the Snoozed view, Spam, Trash, and Outbox retain their dedicated actions.
