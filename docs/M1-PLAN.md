...
3. **Navigation UI:** reuse the current full-width list ⇄ full-window reader/composer. Show `view-title`; keep Important/Other/user splits visible only for Inbox; register `Go to …` palette commands and the complete `G` chords (`I/A/T/D/S/H/P/R`, plus `1`–`9` for splits in configured order). Returning to a view restores its selection and scroll.
...
5. **Context-aware footer:** default hints are a minimal non-wrapping set for the active view. While `G` is pending, replace them with valid mailbox/split completions derived from the command registry until completion, `Esc`, view change, or a 2–3 second timeout. The palette and cheat sheet remain exhaustive.
