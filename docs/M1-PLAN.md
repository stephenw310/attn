...
**Basis:** [SPEC.md](SPEC.md) v0.13 §8 M1 and the shipped implementation.
...
**Revised 2026-08-13 (exit audit):** T12A shipped in PR #24. Three unplanned-but-accepted additions landed after the plan was written: desktop packaging with a manual build workflow (PR #26), the sync status surface with staged/resumable backfill (PR #25), and the signed-out onboarding screen that replaced mock mode (PR #27) — all three are now recorded in SPEC v0.13 (§6 Packaging, F2 "Sync visibility", F1 "Signed-out state", §9 #12). The T12 engineering closeout and real-Gmail airplane-mode smoke are **done**; only the real-OS notification click-through smoke remains. **[M2-PLAN.md](M2-PLAN.md) is the current milestone guide.**
...
3. ~~Execute and record the real-Gmail airplane-mode/relaunch smoke.~~ Verified by the owner on 2026-08-13.
4. Execute the real-OS notification click-through smoke — **the only remaining M1 evidence item; it needs a human with a signed-in build on macOS/Windows.** M2 work is already underway.

---

## Global rules (every task)

1. **The local schema is a disposable development snapshot** (`src/main/db/schema.ts`). Schema changes update the snapshot and bump its version; stale profiles are deleted and re-synced instead of migrated. Migration references in the shipped task records below are historical implementation notes.
2. **IPC has three parts** — a capability is added in `src/main/ipc.ts` (handler), `src/preload/index.ts` (bridge method), and `src/shared/ipc.ts` (channel type). All three land together. The renderer never imports from `src/main/`.
...
One transaction: update `thread_labels`; recompute the denormalized `threads` flags (`is_unread` from UNREAD, `is_starred` from STARRED). Per-message unread state is not stored because the UI and triage model are thread-level. INBOX membership drives `listInboxThreads`, so archive = removing INBOX just works.
...
- Keyboard: `l` opens (register as a command; works from the list or reader). The existing keydown guard already ignores keystrokes when an `INPUT` is focused, so the search field won't trigger verbs. `Esc` closes the picker before it returns from the conversation.
...
## T11 — Reading experience overhaul: interim split view, HTML mail, full message display

**Status: shipped in PR #11; refined in PRs #21/#23; layout shell superseded by T12A.** · **Depends on:** T1/T3/T8 · **Historical spec:** F3 v0.11, §9 #7/#9 · **PRs:** [#11](https://github.com/stephenw310/attn/pull/11), [#21](https://github.com/stephenw310/attn/pull/21), [#23](https://github.com/stephenw310/attn/pull/23).
...
> Historical implementation record: T12A and SPEC v0.12 supersede this layout/focus shell while retaining Parts C–E.
...
- **Historical keyboard behavior:** `Enter` opened into message focus; clicking a row kept list focus; `Esc` closed the pane. `←`/`→` switched focus, and `J`/`K` changed meaning by focus. T12A removes this mode while retaining iframe key forwarding, native link Enter, post-control keyboard continuity, and auto-advance.
...
- PR #21 resolves `cid:` inline images through a typed `mail:getInlineImage` bridge using cached inline bytes or the signed-in attachment path. The endpoint accepts only PNG/JPEG/GIF/WebP MIME types and returns the content to the renderer as a base64 `dataUrl` with a 25 MB decoded-byte ceiling; `MessageBody` assigns that URL to the matching image in the scriptless iframe. Missing or malformed references remain inert broken-image placeholders. Attachment content therefore does cross the bridge for CID rendering even though the stored `inlineData` field is excluded from `ConversationMsg`.
...
- Implementation and review fixes shipped in **PRs #11, #21, and #23**; all merge gates are complete.
- T11's message-display criteria demonstrably hold (recipients inspectable, remote + inline attachment discovery, CID resolution, and safe/stable collapse). Its pane/focus shell is deliberately superseded by SPEC v0.12 and T12A; the system-mailbox criteria remain assigned to M3.
...
1. Opening by `Enter`, click, or notification replaces the list with a full-window conversation. The reader header contains Back/List, subject, queue position (for example, “4 of 12”), and the `Esc` hint. The list DOM may remain hidden to preserve its exact scroll position, but it must not be visible or interactive while reading.
2. Remove `splitFocus`, compact list rows, focus indicators, and left/right pane switching. `Esc` and Back/List return to the selected list row without losing scroll context.
3. While reading, `J`/`K` always changes conversation and resets the reader to the top. Arrow, Space/Shift+Space, and Page keys scroll. Adjacent real conversations remain prefetched into the renderer cache so navigation normally renders without a loading state.
4. Route every semantic keyboard action through the command registry, including `G I`/`G H`; keep reader scrolling plus picker/menu-local navigation, confirmation, and dismissal as explicit non-palette interaction primitives. Unit-test the registry inventory, uniqueness, modifier behavior, aliases, and chord dispatch.
5. Update e2e behavior and all four screenshots. Screenshot setup opens rows with a single click or keyboard action so it cannot leave a text-selection highlight.

### Done when

- SPEC, plan, README, and AGENTS describe only the full-window reading contract as current behavior.
- No split-focus state or split-specific keyboard branch remains in the renderer.
- Registry unit tests and the updated Electron e2e suite pass; all four artifacts are visually clean.
...
4. ⬜ **Manual evidence:** the real-Gmail airplane-mode → quit → relaunch-online drain passed on 2026-08-13. Re-test a real macOS/Windows notification click-through to the intended thread and record the result. **This needs a human with a signed-in build and is the only remaining M1 evidence item.**
...
**Status: planned.** · **Depends on:** T7 incremental sync; M2 for editable Draft rows · **Spec:** F3 (v0.14), F5, §9 #10/#14
...
3. **Navigation UI:** reuse the current full-width list ⇄ full-window reader/composer. Show `view-title`; keep Important, Other, and user splits visible only for Inbox; register `Go to …` palette commands and the fixed-mailbox `G` chords (`I/A/T/D/S/H/P/R`). Use `Tab` and `Shift+Tab` for split navigation. Returning to a view restores its selection and scroll.
4. **View behavior:** switching mailboxes closes the open reader; Drafts merges local composing outbox rows with cached Gmail Drafts and opens either in the full-window composer; local triage refreshes the active filter immediately. T16's on-demand Outbox handles queued/sending/failed/needs-review and is not a permanent mailbox/sidebar entry. Spam/Trash are browsable, but v1 does not add permanent-delete or empty-folder actions.
5. **Context-aware footer:** default hints are a minimal non-wrapping set for the active view. While `G` is pending, replace them with fixed-mailbox completions derived from the command registry until completion, `Esc`, view change, or a 3-second timeout. The palette and cheat sheet remain exhaustive.
6. **Coverage:** seed at least one thread/message per system label. E2e every palette/chord route, label-correct row membership, All Mail exclusion rules, per-view selection/scroll restoration, Draft-to-composer behavior, contextual footer states, and offline cached switching. Unit-test the view-to-query mapping and history-driven membership updates.
...
| Permanently-failed queue rows count toward the pending badge forever; `last_error` has no UI surface and no retry/clear affordance | T3 | **Implemented in [M2 T18](M2-PLAN.md#t18--self-healing-failed-actions-deciding-an-m1-deferred-call):** failed triage actions self-heal to server truth with an explanatory toast — no panel, no retry button |
| Hard 401s mark queue rows `failed` permanently — actions queued across a revoked-token window never retry after re-sign-in | T3 | **Implemented in [M2 T18](M2-PLAN.md#t18--self-healing-failed-actions-deciding-an-m1-deferred-call):** auth failures never revert; they re-pend on the next successful sign-in |
...
- [x] Current `npm run verify` was green at the 2026-08-13 exit audit
...
- [x] SPEC v0.13, M1 plan, README, and AGENTS reflect the shipped M1 behavior
...
M2 (composer, drafts, send + undo send, exactly-once outbox — [M2-PLAN.md](M2-PLAN.md)) is underway.
