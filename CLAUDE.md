...
This is the only file you need to start work. [docs/SPEC.md](docs/SPEC.md) is the source of truth for product behavior — consult it for any feature question. [README.md](README.md) covers human onboarding (prerequisites, Google OAuth client setup); you don't need Google credentials to build or test.
...
- **Mail bodies are untrusted input.** Render them as text nodes only; sanitized HTML rendering is a later milestone (SPEC §6).
