...
Attn — keyboard-first, local-first desktop email client (Electron + React + TS + SQLite).
**SPEC.md is the source of truth** for product behavior; SETUP.md covers dev setup. Currently at M0 (walking skeleton).
...
```
npm run verify     # typecheck (3 tsconfigs) + biome lint + build + e2e
```
