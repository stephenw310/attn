...
test('the visible message cursor targets reply, reply all, and forward shortcuts', async ({
  ...
  await expect(page.getByTestId('message-actions')).toHaveCount(1)
...
test('Tide reader expands messages and gives Escape to the inline draft first', async ({
  page
}, testInfo) => {
  await page.setViewportSize({ width: 900, height: 700 })
  await page.getByTestId('thread-row').first().click()
  const cards = page.getByTestId('message-card')
  const toggleAll = page.getByTestId('conversation-toggle-all')
  await expect(toggleAll).toHaveText('Expand all messages')
  await toggleAll.click()
  await expect(cards.locator('[data-testid="older-message-toggle"][aria-expanded="true"]')).toHaveCount(3)
  await expect(toggleAll).toHaveText('Collapse all messages')
  await toggleAll.click()
  await expect(cards.locator('[data-testid="older-message-toggle"][aria-expanded="false"]')).toHaveCount(3)
  await page.getByTestId('conversation-scroll').focus()
  await page.keyboard.press('p')
  await page.keyboard.press('p')
  await page.keyboard.press('o')
  ...
  await expect(page.getByTestId('composer-not-sent')).toHaveText('Not sent')
  await expect(page.getByTestId('composer-close')).toContainText('Save & close')
  await composer.typeBody('Keep this reply after Escape.')
  await composer.expectSaved()
  for (const colorScheme of ['light', 'dark'] as const) {
    await page.emulateMedia({ colorScheme })
    await expect(page.locator('html')).toHaveAttribute('data-theme-appearance', colorScheme)
    await expect(page.getByTestId('composer-close')).toBeInViewport()
    const dir = join(__dirname, '.artifacts')
    mkdirSync(dir, { recursive: true })
    const path = join(dir, `tide-inline-narrow-${colorScheme}.png`)
    await page.screenshot({ path })
    await testInfo.attach(`Tide inline reply ${colorScheme}`, { path, contentType: 'image/png' })
  }
  await page.keyboard.press('Escape')
  await expect(composer.root).toHaveCount(0)
  await expect(page.getByTestId('conversation-view')).toBeVisible()
  await expect(page.getByTestId('conversation-scroll')).toBeFocused()
  await expect(page.getByTestId('conversation-message').first().getByTestId('message-cursor')).toBeVisible()
  await expect(page.getByTestId('conversation-back')).toContainText('Esc')
  for (const colorScheme of ['light', 'dark'] as const) {
    await page.emulateMedia({ colorScheme })
    await expect(page.locator('html')).toHaveAttribute('data-theme-appearance', colorScheme)
    const dir = join(__dirname, '.artifacts')
    mkdirSync(dir, { recursive: true })
    const path = join(dir, `tide-reader-narrow-${colorScheme}.png`)
    await page.screenshot({ path })
    await testInfo.attach(`Tide reader ${colorScheme}`, { path, contentType: 'image/png' })
  }
  await page.keyboard.press('Escape')
