...
test('preserves differently styled propagated decorations from clipboard CSS', async ({ page }) => {
  ...
  expect(html).toMatch(/background-color:[^;"]*(?:yellow|255, 255, 0)/)
...
test('keeps physical-size flex siblings responsive after paste', async ({ page }) => {
  const composer = new ComposerPage(page)
  await composer.openNew()
  await composer.editor.click()
  await pasteHtml(
    composer,
    '<style>.row{display:flex}.fixed{width:200px;flex-shrink:0}.fluid{flex:1}</style><div class="row"><div class="fixed">Fixed</div><div class="fluid">Fluid</div></div>'
  )
  const fluid = composer.editor.frameLocator('iframe').getByText('Fluid', { exact: true })
  const sizes = await fluid.evaluate((element) => {
    let item = element as HTMLElement
