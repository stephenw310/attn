...
- Build the app with `npm run build`. `scripts/doctor.mjs` passes.
- Every drive seeds `e2e/fixtures/seed-inbox.json` unless its preconditions name another fixture. That seed signs in `seed@attn.test` with 8 inbox rows, 4 of them unread. Row order from the top is `Q3 roadmap review` (Maya Lin), `Your receipt` (Northstar Books), `Design notes` (Theo Park), `Lunch next week`, `August budget`, `Flight options`, `Research summary`, and `This week in focus`.
- The app boots in the Inbox list with the first row selected.
- `assertIsolated(app, userData, mainLog)` passes at the start of the drive.
- Never drive an instance that this run did not start.

## Driving conventions

- Start every recipe from the baseline state unless its preconditions say otherwise.
- Select with `page.getByTestId(...)` and the handles named in the recipe. Do not select by class or DOM position.
- Press keys with `page.keyboard.press(...)`. Treat every key and string as literal.
- Wait with `expect(...)` or `expect.poll(...)`. Do not sleep.
- A mutation leaves a queued action. Read `pending-count` as part of the result, not as an error.

## Proof and skip reporting

- Capture the user action and the resulting state with numbered `snap()` calls.
- Pair every UI claim with a store read through the production bridge (`window.attn.mail.*`, `window.attn.settings.*`) or a `mainLog()` match, saved with `record()`.
...
3. `Driving it with a verify-attn drive` starts with `Preconditions:` and uses labeled bullets that pair each user action with the exact Playwright call and the observable result.
4. `Gotchas` lists traps that can waste or invalidate a verification run.

Keep implementation details out of the map. Name only user paths, stable handles, required state, calls, and observable proof.
