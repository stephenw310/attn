...
it('projects virtual rows into the space left by an exiting row without moving the cursor', async () => {
  ...
  try {
    ...
    expect(secondRow?.closest<HTMLElement>('.absolute')?.style.top).toBe('98px')
...
it('measures the sizer against the list when scrolling a selection into view', async () => {
  ...
  try {
    ...
    // Row 3 sits at 206 in layout coordinates and is 54 tall, so pulling its
    // bottom to the viewport floor lands at 8 + 206 + 54 - 100. Reading the
    ...
    expect(list?.scrollTop).toBe(LIST_PADDING_TOP + 206 + 54 - LIST_CLIENT_HEIGHT)
  } finally {
    await act(async () => root.unmount())
